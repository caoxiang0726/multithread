...
class ThreadA extends Thread{
    ...
    public void run() {
        synchronized (this) {
            System.out.println(Thread.currentThread().getName()+" call notify()");//3 t1 call notify()
            // 唤醒当前的wait线程
            notify();
        }
    }
...
public class WaitTest {
    ...
    public static void main(String[] args) {
        ...
        synchronized(t1) {
            try {
                // 启动“线程t1”
                System.out.println(Thread.currentThread().getName()+" start t1");//1 main start t1
                t1.start();

                // 主线程等待t1通过notify()唤醒。
                System.out.println(Thread.currentThread().getName()+" wait()");//2 main wait()
                t1.wait();//让主线程阻塞

                System.out.println(Thread.currentThread().getName()+" continue");//4
 ...
 * 流程：
 * (01)
 (02) “主线程”通过 new ThreadA("t1") 新建“线程t1”。
        随后"主线程"通过synchronized(t1)获取“t1对象的同步锁”。然后调用t1.start()启动“线程t1”。
 ...
 *
 ...
 换句话说，这个方法和wait(0)的效果一样！(补充，对于wait(long millis)方法，当millis为0时，表示无限等待，
 直到被notify()或notifyAll()唤醒)。
